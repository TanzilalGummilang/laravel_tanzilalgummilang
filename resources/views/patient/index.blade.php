          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <button type="button" class="mb-3 btn btn-sm btn-primary btn-add">
                <i class="ti-plus"> Tambah Pasien</i>
              </button>
              {{ $dataTable->table() }}
...
@push('js')
  ...
  <script>
    ...
    $('.btn-add').on('click', function() {
      $.ajax({
        ...
        success: function(res) {
          ...
          modalAction.show()
          $('#modalActionLabel').text('Tambah Data Pasien')
    ...
    // btn edit & delete
    $('#patient-table').on('click', '.action', function() {
      let data = $(this).data()
      let id = data.id
      let type = data.type
      console.log(data)
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              ...
              success: function(res) {
                window.LaravelDataTables["patient-table"].ajax.reload()
                Swal.fire(
                  'Terhapus!',
                  res.message
                )
      ...
      $.ajax({
        method: 'get',
        url: `{{ url('patients/') }}/${id}/edit`,
        success: function(res) {
          $('#modalAction').find('.modal-dialog').html(res)
          modalAction.show()
          $('#modalActionLabel').text('Edit Data Pasien')
